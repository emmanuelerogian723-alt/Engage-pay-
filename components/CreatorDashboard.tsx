...
const CreateTaskModal: React.FC<{ isOpen: boolean, onClose: () => void, onCreateCampaign: (campaign: Omit<Campaign, 'id' | 'creatorId' | 'completedTasks' | 'status'>) => void, balance: number }> = ({ isOpen, onClose, onCreateCampaign, balance }) => {
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const totalTasks = Number(formData.get('totalTasks'));
        const payout = Number(formData.get('payout'));
        const budget = totalTasks * payout;
        
        if (budget > balance) {
            alert('Insufficient funds. Please fund your account to launch this campaign.');
            return;
        }

        const newCampaign = {
            name: formData.get('campaignName') as string,
            platform: formData.get('platform') as SocialPlatform,
            engagementType: formData.get('engagementType') as EngagementType,
            budget: budget,
                    ...
                    <div className="p-6">
                        <div className="space-y-4">
                            <div>
                                ...
                                <input name="campaignName" type="text" placeholder="e.g., Summer Collection Launch" required className="block w-full bg-gray-100 dark:bg-gray-700 border-transparent rounded-md p-2 focus:ring-primary-500 focus:border-primary-500" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Platform</label>
                                    <select name="platform" required className="mt-1 block w-full bg-gray-100 dark:bg-gray-700 border-transparent rounded-md p-2 focus:ring-primary-500 focus:border-primary-500">
                                ...
                                <div>
                                    ...
                                    <select name="engagementType" required className="mt-1 block w-full bg-gray-100 dark:bg-gray-700 border-transparent rounded-md p-2 focus:ring-primary-500 focus:border-primary-500">
                            ...
                            <div className="flex space-x-4">
                               <div className="flex-1">
                                    ...
                                    <input name="payout" type="number" step="0.01" placeholder="$0.10" required className="mt-1 block w-full bg-gray-100 dark:bg-gray-700 border-transparent rounded-md p-2 focus:ring-primary-500 focus:border-primary-500" />
                                </div>
                                 <div className="flex-1">
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Total Tasks</label>
                                    <input name="totalTasks" type="number" placeholder="1000" required className="mt-1 block w-full bg-gray-100 dark:bg-gray-700 border-transparent rounded-md p-2 focus:ring-primary-500 focus:border-primary-500" />
                                </div>
                            </div>
                        </div>
                    </div>
